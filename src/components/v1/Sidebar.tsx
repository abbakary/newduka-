...
import { ChevronRight, LogOut, Wifi, WifiOff, X } from 'lucide-react';
    ...
    <>
      ...
      <aside
        className={`
          ...
          w-[min(17.5rem,88vw)] lg:w-[15.5rem] lg:min-w-[15.5rem]
          m-0 lg:m-3 lg:mr-0
      ...
      >
        <div className="p-4 border-b border-white/10 flex items-start justify-between gap-2">
          <div className="flex items-center gap-3 min-w-0">
            <BrandLogo height={36} className="rounded-lg brightness-110 shrink-0" />
          ...
          {onMobileClose && (
            <button
              type="button"
              onClick={onMobileClose}
              className="lg:hidden p-1.5 rounded-lg text-white/70 hover:text-white hover:bg-white/10 cursor-pointer shrink-0"
              aria-label={isSw ? 'Funga menyu' : 'Close menu'}
            >
              <X className="w-5 h-5" />
            </button>
        ...
        {staffRole && (
          <p className="px-4 pt-2 text-[10px] font-bold uppercase tracking-wider text-white/50">
            {staffRole}
          </p>
        )}
