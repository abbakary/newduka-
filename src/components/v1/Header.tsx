...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <div className="flex flex-col select-none font-['Calibri',_'Aptos',_'Segoe_UI',_sans-serif]">
      ...
      {isImpersonating && (
        <div className="bg-amber-600 text-white px-3 sm:px-6 py-2 text-[11px] sm:text-xs font-semibold flex flex-col sm:flex-row sm:items-center justify-between gap-2 shadow-md z-30 animate-in slide-in-from-top-2">
          <div className="flex items-start sm:items-center gap-2 min-w-0">
            <span className="w-2.5 h-2.5 rounded-full bg-white animate-ping shrink-0 mt-1 sm:mt-0"></span>
            <span className="min-w-0">
              <strong>{isSw ? 'HALI YA UIGAJI:' : 'IMPERSONATION ACTIVE:'}</strong>{' '}
              {isSw ? 'Unatazama duka la' : 'Operating:'} <u className="break-words">{impersonatedTenantName || (isSw ? 'Biashara Yako' : 'Your Business')}</u>
            </span>
          </div>

          <button
            onClick={onExitImpersonation}
            className="flex items-center justify-center gap-1.5 px-3 py-1 rounded-lg bg-slate-950 hover:bg-black text-amber-300 hover:text-white text-xs font-bold transition-all cursor-pointer shadow-xs shrink-0 self-stretch sm:self-auto"
          >
      ...
      >
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 min-w-0 flex-1">
          ...
          {isSuperAdmin ? (
            <div className="flex items-center gap-2 min-w-0">
              <span className="px-2 py-1 rounded-xl bg-emerald-50 text-emerald-800 font-mono text-[10px] sm:text-xs font-bold border border-emerald-300 flex items-center gap-1.5 shadow-2xs truncate">
            ...
            <div className="flex items-center gap-2 min-w-0 flex-1">
              <div className="relative w-full max-w-[14rem] sm:max-w-none sm:w-44 md:w-52 lg:w-60 xl:w-64 min-w-0">
                <Search className="w-3.5 h-3.5 text-[#605E5C] absolute left-2.5 top-1/2 -translate-y-1/2" />
                <input
                  id="global-search-input"
                  type="text"
                  placeholder={isSw ? 'Tafuta…' : 'Search…'}
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div
            className={`flex items-center gap-1.5 px-2 py-1.5 rounded-xl text-xs font-bold border transition-all shadow-2xs shrink-0 ${
              isOnline
          ...
          >
            ...
            <span className="hidden md:inline">{isOnline ? (isSw ? 'Mtandaoni' : 'Online') : (isSw ? 'Nje ya mtandao' : 'Offline')}</span>
          </div>
